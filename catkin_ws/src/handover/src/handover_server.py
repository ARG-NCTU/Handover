...
import random
import numpy as np
sys.path.append('/home/dualarm/Handover/catkin_ws/src/ros_handover/handover/src/HANet')
...
class HandoverServer:
    ...
    def generate_view(self, Cl, Dl, CR, DR, id):
        pcl_l, _ = get_pcd_left(Cl, Dl)
        pcl_r, _ = get_pcd_right(CR, DR)

        image, depth = get_view(pcl_r, pcl_l, id)

        return image, depth

    def single_pred(self, c, d, armm):
        target1, _, dis1, aff_map1, v1, a, _ = self.pred.predict_mul(c, d, armm)
        return target1, aff_map1, dis1, a

    def multi_pred(self, c90, d90, c67, d67, c45, d45, c22, d22, c0, d0):
        aff_map90, v90, a90, center90 = self.pred.predict_mul(c90, d90, 'right_arm', True)
        aff_map67, v67, a67, center67 = self.pred.predict_mul(c67, d67, 'right_arm', True)
        aff_map45, v45, a45, center45 = self.pred.predict_mul(c45, d45, 'right_arm', True)
        aff_map22, v22, a22, center22 = self.pred.predict_mul(c22, d22, 'right_arm', True)
        aff_map0, v0, a0, center0 = self.pred.predict_mul(c0, d0, 'right_arm', True)
    ...
    def execute_cb(self, msg):
        ...
        # Go target
            ...
            stop = self.get_pose()
            print(stop.target_pose.position.x,start.target_pose.position.x)
